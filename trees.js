...
function findFarNeighbors(tree) {
    const allNeighbors = findNeighborsDeep(tree.index, tree.size, []);
    const nearNeighbors = findNeighborsDeep(tree.index, 1, []);
    return allNeighbors.filter(s => !nearNeighbors.includes(s));
}
...
function findParentTree(treeSet, target) {
    ...
    for (let i = 0; i < newTreeSet.length; i++) {
        console.error("tree: " + JSON.stringify(newTreeSet[i]));
...
while (true) {
    ...
    for (let i = 0; i < numberOfTrees; i++) {
        ...
        if (isMine) {
            t.neighbors = findFarNeighbors(t);
        }
        trees.push(t);
    }
    ...
    for (let i = 0; i < 1; i++) {
        ...
        const allUsableTrees = usableBigTrees.concat(usableMediumTrees);

        //if (usableBigTrees.length > 0 && sun >= 4 && day >= 12 && ((dayIter < 1 && (day%2==0)) || day >= 23 || )) {
        if (usableBigTrees.length > 0 && sun >= 4 && ( day >= 23 || usableBigTrees.length > 4 || oppHarvested)) {
            oppHarvested = false;
        ...
        if (!setCommand && allUsableTrees.length > 0 && sun >= (seedTrees.length) && day < 20 && seedTrees.length===0) {
